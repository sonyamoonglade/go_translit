...
func EncodeWord(word string) string {
	if strings.Contains(word, " ") {
		return strings.Join(Encode(strings.Split(word, spaceSeparator)), " ")
	}
	return encodeWord(word)
}
...
func DecodeWord(word string) string {
	if strings.Contains(word, " ") {
		return strings.Join(Decode(strings.Split(word, spaceSeparator)), " ")
	}
	return decodeWord(word)
}
...
func decodeWord(word string) string {
	...
	if isNotEncoded(word) {
		return word[len(noEncPrefixStr):]
	}
	return fn.Compose2(byteSliceToStr, decodeFromAscii)(word)
}

func isEnglishOrNumericalOrSpecial(word string) bool {
	for _, ch := range word {
		if !(('a' <= ch && ch <= 'z') || ('A' <= ch && ch <= 'Z') || unicode.IsDigit(ch) || (unicode.IsPunct(ch))) {
			return false
		}
	}
	return true
